...
sub _guess_ntok {
   ...
   for my $i ($sidx .. $sidx + $slen - 1) {
      next if ($tlen += length($rtok->[$i])) < $slen;
      return $i-$sidx if $tlen > $slen;
      return $i-$sidx + (${$rexact} = 1);
   }
   $slen;
}
...
sub _pour_chunk {
   ...
   $idot >= 0 && $r3 == 0 and return join("", @{$rtok}[$sidx .. $idot-1],
      ".''" x $i3, @{$rtok}[$idot .. $eidx]);
   my $i2 = $diff >> 1; my $r2 = $diff & 1;
   if ($r2 == 0 and $iquote >= 0 || $idollar >= 0) {
      $iquote = $idollar if $iquote < 0;
      return join("", @{$rtok}[$sidx .. $iquote-1],
         '(' x $i2 . $rtok->[$iquote] . ')' x $i2,
         @{$rtok}[$iquote+1 .. $eidx]);
   }
   $i3quote >= 0 and return join("", @{$rtok}[$sidx .. $i3quote-1],
      $diff == 1 ? '"\\' . substr($rtok->[$i3quote], 1, 1) . '"' :
...
sub pour_sightly {
   ...
   @{$rfillv} > @filleqto and die "too many rfillv";
   @filleqto % @{$rfillv} and splice(@filleqto, -(@filleqto % @{$rfillv}));
   my @filler = (); my $vi = -1; my $ttlen = 0;
   for my $e (@filleqto) {  # 11 is length of each (see 11 below)
   ...
   push(@ptok, (@filler) x (int($ttlen/(11 * @filleqto)) + 1));
   my $sidx = my $nshape = 0; my $exactfit;
   while (1) {
...
sub make_triangle {
   my $w = $_[0]->{Width}; $w & 1 or ++$w; $w < 9 and $w = 9;
   my $n = $w >> 1; my $s;
...
sub make_siertri {
   my $w = $_[0]->{Width}; $w < 3 and $w = 5; my $n = 2 ** $w; my $s;
   for my $i (0 .. $n-1) {
      --$n; $s .= ' ' x $n .
      join('', map($n & $_ ? '  ' : '##', 0 .. $i)) . "\n";
   }
...
sub make_banner    { _make_banner($_[0]->{Width}, $_[0]->{BannerString}) }
sub make_srcbanner { _make_banner($_[0]->{Width}, $_[0]->{SourceString}) }
...
my %builtin_shapes = (
   'triangle'   => \&make_triangle, 'siertri'    => \&make_siertri,
   'banner'     => \&make_banner,   'srcbanner'  => \&make_srcbanner
);
...
sub sightly {
   ...
   my $shape = my $sightly = "";
   $arg{SourceString} and $sightly = $arg{Print} ?
...
F<mkeye.pl>:
    ...
    my $orig; { local $/ = undef; $orig = <F> } close(F);
...
Some background on shape I<gelly> can be found at
