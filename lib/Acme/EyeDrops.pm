...
# ${$rstr) of length $slen. Return 1 if successful, else 0.
sub _pour_line {
   my ($rtok, $sidx, $n, $slen, $rstr) = @_;
   my $eidx = $sidx + $n - 1; my $tlen = 0;
   my $idot = -1; my $iquote = -1; my $i3quote = -1;
   my $iparen = -1; my $idollar = -1;
   for my $i ($sidx .. $eidx) {
      $tlen += length($rtok->[$i]);
      if ($rtok->[$i] eq '.') { $idot = $i }
   ...
   if ($tlen == $slen) {
      ${$rstr} = join("", @{$rtok}[$sidx .. $eidx]); return 1;
   }
   return 0 if $tlen > $slen;
   my $diff = $slen - $tlen;
   my $i3 = int($diff/3); my $r3 = $diff % 3;
   if ($idot >= 0 && $r3 == 0) {
   ...
   # ouch, can't test for nexttok eq '(' in case next line also adds '+'
   if ($rtok->[$eidx] ne '=' and
...
sub pour_sightly {
   ...
   scalar(@{$rfillvar}) > scalar(@filleqto) and die "too many rfillvar";
   ...
   push(@ptok, (@filler) x (int(length($tlines)/$filllen) + 2));
         ...
         for my $it (0 .. $#{$rline}) {
            ...
            if ($plen == $tlen) {
               ...
               if ($exactfit) {
                  ...
                  while ($n > 0) {
                     my $b = $fcompact ?
                     _pour_compact_line(\@ptok, $sidx, $n, $tlen, \$str) :
...
sub sightly {
   ...
   if ($shapestr) {
      ...
      if ($arg{BorderGap}       or $arg{BorderWidth}       or
         ...
         $widthbottom = $arg{BorderWidthBottom} if $arg{BorderWidthBottom};
...
The F<.eye> file shapes distributed with this version of EyeDrops are:
    ...
    china1       Chinese characters related to Christmas
...
The cam.pm Christmas Perl Programming Contest at
F<http://www.cam.pm.org/archive/2002-December/000953.html>.
