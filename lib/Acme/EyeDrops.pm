...
sub _get_eye_keywords {
   my $d = shift;
   my %h;
   for my $s (_get_eye_shapes($d)) {
      my $p = _get_eye_properties($d, $s) or next;  # no properties
      exists($p->{keywords})              or next;  # no keywords property
      my @k = split(" ", $p->{keywords})  or next;  # no keywords
...
Thanks to cog for the new Shape Properties feature.
...
Copyright (c) 2001-2005 Andrew Savige. All rights reserved.
