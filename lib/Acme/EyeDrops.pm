...
@EXPORT_OK = qw(ascii_to_sightly sightly_to_ascii
   ...
   reduce_shape expand_shape pour_text pour_sightly sightly);
$VERSION = '1.30';
...
sub pour_text {
   ...
   my $txtend = length($txt);
   my $nshape = int(length($txt)/$ttlen);
   my $rem = length($txt) % $ttlen;
   if ($rem != 0 || $txt eq "") {
      ++$nshape;
...
sub pour_sightly {
   ...
   my @filler = (); my $vi = -1; my $ttlen = my $sidx = 0;
...
precisely into the provided snowflake shape. You can pour the above
program into the required shape with:

    sightly.pl -s snow -f snowing.pl -t -u# -n1 >snowflake.pl

which produces a valid entry, F<snowflake.pl>:
...
FILLTEXT is text to be used as a filler for any leftover
part of the shape (if not set, don't fill in leftovers).
...
The attributes that HASHREF may contain are:
    ...
    TextFiller    Filler used with Text attribute.
