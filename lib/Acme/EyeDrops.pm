...
sub pour_sightly {
   ...
   $tlines =~ s/ +$//mg; my @tnlines = ();
   for my $line (split(/\n/, $tlines)) {
      if ($line =~ /^\s*$/) { push(@tnlines, undef); next }
      my @oneline = ();
      my @ttok = $line =~ / +|[^ ]+/g;
   ...
   if ($prog) {
      ...
      # Tokenize valid ascii_to_sightly() output.
      push(@ptok, $prog =~ /[().&|^]|'\\\\'|.../g);  # ... is "'"|'.'
   }
 ...
 /[()&|^=;]|\$.|'[^'\\]*(?:\\.[^'\\]*)*'|"[^"\\]*(?:\\.[^"\\]*)*"/g;
         if (@t) {
            ...
            for my $c (@t) {
               $c eq '(' and ++$nl; $c eq ')' and ++$nr;
               $c eq '=' and ++$ne;
            }
            if ($nl != $nr or $t[-1] eq '=' or $ne == 0) {
