...
sub sightly {
   ...
   if ($arg{ShapeString}) {
      ...
      $shape = join("\n" x $arg{Gap}, map(exists($builtin_shapes{$_}) ?
         $builtin_shapes{$_}->(\%arg) : (m#[./]# ? _slurp_tfile($_) :
...
sub get_eye_string { _slurp_tfile("$eye_dir/$_[0].eye") }
...
The sightly function returns a properly shaped program string;
there is no error return. If something is badly wrong, die is called.
So if you are calling sightly in an environment where it's
unacceptable to die, be sure to wrap the sightly call in
an eval block. For example:
...
F<http://terje.perlgolf.org/>. Ton Hospel's excellent generic
...
Copyright (c) 2001-2004 Andrew Savige. All rights reserved.
